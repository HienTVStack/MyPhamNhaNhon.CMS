                ...
                {discounts?.map((item, index) => (
                  <TableRow key={index}>
                    ...
                    <TableCell>
                      ...
                      <Typography variant="body2">Thể loại: {item.type === 1 ? "Cho khách hàng" : "Cho tổng hóa đơn"}</Typography>
                    </TableCell>
                    <TableCell>{fDate(item.createdAt)}</TableCell>
                    <TableCell>{fDate(item.finishedAt)}</TableCell>
                    {/* <TableCell>{`${item.valueList[]discountValue} %`}</TableCell> */}
                    <TableCell>{`${item?.valueList[item?.valueList.length - 1]?.discountValue || ""} %`}</TableCell>
                    <TableCell
                      sx={{
